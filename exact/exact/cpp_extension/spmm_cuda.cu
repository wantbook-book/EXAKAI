...
// each thread will get the value passed from its next delta thread
...
// tell compiler unroll this loop automatically
#pragma unroll
      for (int i = 0; i < 32; i++) {
...
spmm_cuda(torch::Tensor rowptr, torch::Tensor col,
  ...
  mat = mat.contiguous();
  // convect IntList to std::vector<int64_t>
