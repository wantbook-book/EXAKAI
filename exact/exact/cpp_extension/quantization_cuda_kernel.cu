...
__global__ void pack_single_precision_kernel(int32_t bits,
  ...
  // block id in thread grid
  const int64_t no = blockIdx.x + block_idx_y_base;
  // thread id in one thread block
  ...
  // 10 rounds
  // get 4 random number each time 
  curandStatePhilox4_32_10_t state;
  // different threads have different serials of random number
  // now you can use state to generate random number
  curand_init(seeds.first, global_thread_id, seeds.second, &state);

  uint8_t local_packed = 0;
  // unroll work_per_thread loop 
  for (int ni = 0; ni < work_per_thread; ni++) {
    // which row of data
    const int64_t n = no * work_per_thread + ni;
    // N is the total number of data row
    if (boundary_check && n >= N) { break; }
    // which element of data
...
Tensor pack_single_precision_cuda(Tensor data,
  ...
  // Compute total bits
  // the number of elements will one thread process
  int work_per_thread = 8 / bits;
  // bits need to be 1, 2, 4, 8
  TORCH_CHECK(8 % bits == 0);
  // why round? make each thread have the same work load
  ...
  // uint8 tensor to store the compressed value 
  ...
  {
    // this block of code won't be executed by multi threads
    // See Note [Acquire lock when using random generators]
    std::lock_guard<std::mutex> lock(gen->mutex_);
    // the total number of random number need to generate: group_size*work_per_thread
    ...
    if (N % work_per_thread == 0) { 
      AT_DISPATCH_FLOATING_TYPES_AND_HALF(data.scalar_type(), "pack_single_precision", ([&] {
        pack_single_precision_kernel<scalar_t, false><<<block_dim, thread_dim>>>(
          ...
          // just use int8? will not overflow?
...
Tensor unpack_single_precision_cuda(Tensor data,
  ...
  // to help calculate the max number of blocks gpu can hold
  ...
  // here sm number * blocks per sm -> total blocks per gpu
  // what to do if max blocks is not enough for num_blocks needed to unpack all elements. kernel has loop
  grid.x = std::min((unsigned int)at::cuda::getCurrentDeviceProperties()->multiProcessorCount * blocks_per_sm, grid.x);

  if (N % work_per_thread == 0) {
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(scale.scalar_type(), "unpack_single_precision", ([&] {
      // 0 represents the size of dynamic shared memory, here no shared memory allowed
...
__global__ void act_quantized_relu_forward_kernel(const scalar_t* __restrict__ data,
  ...
  // now we have 512 threads per block.one bit for one thread
  // threads within one SM can share the same mask
  ...
  if (threadIdx.x * 2 < shared_len) {
    // init data to 0
    // int2 8bits? should be 64bits: speed up IO faster than two ints
...
__global__ void act_quantized_dropout_forward_kernel(at::cuda::detail::TensorInfo<scalar_t, int64_t> a,
  ...
  for (int64_t linearIndex = id;
      ...
      //curand_uniform_double was pure evil anyway, not doing what it promises, and there's nothing for halfs, so generate float for everything
       ...
       rand.z = rand.z > p;
       rand.w = rand.w > p;
       // init data to 0, low efficiency "if"
       if (threadIdx.x * 2 < shared_len) {
...
std::pair<Tensor, Tensor> act_quantized_dropout_forward_cuda(Tensor data, float p) {
  ...
  grid.x = std::min((unsigned int)at::cuda::getCurrentDeviceProperties()->multiProcessorCount * blocks_per_sm, grid.x);
  // how many elements is processed within one thread
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(data.scalar_type(), "act_quantized_dropout_forward", ([&] {
    ...
    // remove dimension with size being 1
