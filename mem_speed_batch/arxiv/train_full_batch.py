...
def test(model, data, evaluator, n_classes, use_labels=False, amp_mode=False):
    ...
    if use_labels:
        ...
        feat = data.x
    # start a context to do mixed precision training, speed up training
    with autocast(enabled=amp_mode):
        # TODO: what's the out shape?
        out = model(feat, data.adj_t)
    # the index of max element on every row on last dimension
...
def main():
    ...
    assert args.model.lower() in ['gcn', 'sage', 'gat', 'gcn2', 'tritongcn']
    # simulate, TODO: what is the difference
    if args.simulate:
        print('=' * 30 + 'Simulation' + '=' * 30 )
        config.simulate = True
    # compress activation or not?
    if args.act_fp and args.kept_frac == 1.0:
        ...
        # FP32 if don't compress activation else INTn, n is set by me
    ...
    # TODO: I don't know what is `qmodule`
    ...
    # TODO: amp: it seems to be memory compression method of dgcn
    # amp: automatic mixed precision
    # GradScaler is used to avoid gradient underflow, first scale the small gradients , 
    # and when backpropagation, scale the gradients back
    if args.amp:
        config.amp = True
        scaler = GradScaler()
    else:
    ...
    # random training, slow down trainning!
    # random number used in SGD?
    if args.seed is not None:
        random.seed(args.seed)
        torch.manual_seed(args.seed)
        # tell cudnn to use deterministic convolution method
    ...
    if model_config['loop']:
        ...
        # usually adj_t is a sparse tensor, and set_diag modify the value of its diagonal elements
        # default value is 1
        # like it says, add self loops to the graph
    ...
    if args.debug_mem:
        ...
        # why do like this? is not the total memory be the after_backward?
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + model_config['epochs']):
            ...
            if args.amp:
                ...
                loss.backward()
                if grad_norm is not None:
                    # if the maximum weight exceeds grad_norm, then multiply each weight with grad_norm / max_weight
                    torch.nn.utils.clip_grad_norm_(model.parameters(), grad_norm)
                optimizer.step()
            # ===========================
